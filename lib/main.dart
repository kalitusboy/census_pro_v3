        ...
        child: InAppWebView(
          ...
          onWebViewCreated: (controller) {
            ...
            controller.addJavaScriptHandler(
              ...
              callback: (args) async {
                final record = Map<String, dynamic>.from(args[0] as Map);
            ...
            controller.addJavaScriptHandler(
              handlerName: 'getImageDataUrl',
              callback: (args) async {
                final filePath = args.isNotEmpty ? args[0]?.toString() ?? '' : '';
                final dataUrl = await DB.imagePathToDataUrl(filePath);
                return {"status": dataUrl.isNotEmpty ? "ok" : "empty", "dataUrl": dataUrl};
              },
            );
            ...
            controller.addJavaScriptHandler(
              ...
              callback: (args) async {
                final stats = Map<String, dynamic>.from(args[0] as Map);
                await exportStatisticsToExcel(stats);
                return {"status": "exported"};
              },
            );
          },
